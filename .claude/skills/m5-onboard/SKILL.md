...
5. **Install apps** (optional, `install_apps.py`) — paste-mode REPL upload of every `.py` from a source directory into `/flash/`, then reboot via `repl_reset` (DTR/RTS is a no-op on native USB — don't reach for it). Source layout: root `*.py` → `/flash/`, `apps/*.py` → `/flash/apps/` (UIFlow's stock launcher scans that). When the bundle ships a root `main.py`, `install_apps.py` also sets NVS `boot_option=0` so UIFlow returns directly to our `main.py` without its own launcher or pre-main network phase.
...
- **NVS writes must match the reader's type.** `boot_option` is read with `get_u8()` and must use `set_u8()`; UIFlow's string settings use `get_str()`/`set_str()`. ESP-IDF tags entries by type, so a blob-tagged key is not interchangeable even when the bytes look identical.
...
- **Cardputer-Adv custom launchers require NVS `boot_option=0`, not 2.** On UIFlow 2.5.1 the installed source defines 0 as `BOOT_OPT_NOTHING`, 1 as the stock menu/network flow, and 2 as network-only setup before `main.py`. Option 2 was physically reproduced with an unresponsive custom-launcher keyboard; changing only the NVS value to 0 restored arrows, Enter, and Snake. The stock option 1 also delivered raw keyboard events, proving the TCA8418 hardware and firmware. Keep option 0 for a root `main.py`; individual apps own their network connections.
...
- **Per-app networking.** The launcher stays radio-neutral and labels WiFi as app-owned. Pense Bem reads its private NVS settings only after ONLINE is selected; offline apps do not wait for a venue network that may not exist.
